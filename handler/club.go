...
func GetClubs(db *sqlx.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		query := `SELECT uuid, id, name, slug, logo_url, banner_url, city, address, bio, status, created_at, updated_at 
		          FROM clubs WHERE (status = 'active' OR status = 'verified')`
		countQuery := `SELECT COUNT(*) FROM clubs WHERE (status = 'active' OR status = 'verified')`
...
func GetClubByID(db *sqlx.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		err := db.Get(&club, "SELECT * FROM clubs WHERE (uuid = ? OR slug = ?) AND (status = 'active' OR status = 'verified')", id, id)
		if err != nil {
